...
from .models import Usr  # Adjust the import based on your project structure
from .serializers import RegisterSerializer, LoginSerializer, PasswordResetSerializer, PasswordResetConfirmSerializer
from django.contrib.auth.tokens import PasswordResetTokenGenerator
from django.conf import settings
...
class AuthViewSet(viewsets.ViewSet):
    ...
    def login(self, request):
        ...
        # Retrieve the user by either email or username
        user = None
        if email:
            try:
    ...
    def password_reset_request(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        email = serializer.validated_data['email']
        user = Usr.objects.filter(email=email).first()
